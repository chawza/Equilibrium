...
e2e/
├── CLAUDE.md           # This file
├── helpers.sh          # Shared setup + helper functions (sourced by page scripts)
├── run-all.sh          # Orchestrator — runs all 5 page suites
├── dashboard/          # Tests 01–04 (Dashboard page: /)
│   └── run.sh
├── budget-form/        # Tests 05–14 (Budget Form page: /budget/[id])
│   └── run.sh
├── tags/               # Tests 15–20 (Tags page: /tags + TagDetail)
│   └── run.sh
├── stats/              # Tests 21–22 (Stats page: /stats)
│   └── run.sh
├── settings/           # Tests 23–26 (Settings page: /settings)
│   └── run.sh
...
# Or run a single page suite:
EQUILIBRIUM_DB=/tmp/eq-test.db bash e2e/dashboard/run.sh
EQUILIBRIUM_DB=/tmp/eq-test.db bash e2e/budget-form/run.sh
EQUILIBRIUM_DB=/tmp/eq-test.db bash e2e/tags/run.sh
EQUILIBRIUM_DB=/tmp/eq-test.db bash e2e/stats/run.sh
EQUILIBRIUM_DB=/tmp/eq-test.db bash e2e/settings/run.sh
```
...
Add the function to the appropriate page script (`dashboard/run.sh`, `budget-form/run.sh`, etc.).
...
In the same page script, add `run_test test_XX_description` at the bottom.
