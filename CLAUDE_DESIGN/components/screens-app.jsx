...
function Stats({ budgets }) {
  ...
  const displayEntries = effectiveSelected
    .map(t => [t, tagTotals[t] || 0])
    .sort((a, b) => b[1] - a[1]);
  const displayMax = displayEntries.reduce((m, [, a]) => Math.max(m, a), 0) || 1;
  const displayTotal = displayEntries.reduce((s, [, a]) => s + a, 0);
            ...
            {displayEntries.map(([tag, amount]) => (
              <div key={tag} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ flex: 1, position: 'relative', height: 20, borderRadius: 4, background: 'hsl(var(--secondary))' }}>
                  <div style={{
                    height: '100%',
                    width: `${(amount / displayMax) * 100}%`,
                    borderRadius: 4,
                    background: getTagBgColor(tag),
                    border: `1px solid ${getTagBarColor(tag)}20`,
...
Object.assign(window, { Stats, StatTile, StackedBar });
...
/* ═══════════════════════════════════════
   ...
   ═══════════════════════════════════════ */
const { useState: useStateApp, useEffect: useEffectApp, useCallback: useCbApp, useRef: useRefApp } = React;
...
function App() {
  ...
  const [page, setPage] = useStateApp(() => {
    return localStorage.getItem('eq_page') || 'dashboard';
  });
  ...
  // Provide tweaks context
  const tweakContext = { recordStyle: tweaks.recordStyle, showTagsInList: tweaks.showTagsInList };
